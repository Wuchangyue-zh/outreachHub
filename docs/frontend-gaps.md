...
已核实：`CSVImport` 提交后进入 `importing` 独立状态，原表单不再渲染，并显示 loading spinner 与“导入中”文案，可防止重复提交。
...
已修复：新增 `/about`，承接公司介绍、招聘、合作伙伴与联系入口；联系邮箱使用 `mailto:`，未配置的社交账号不再展示空链接。
...
D1/D7/D3/D5/D2/D4/D8 已完成，D6 已完成（§9.51），D9 为后续优化项。
