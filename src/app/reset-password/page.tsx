  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.message || data.error?.message || t('resetPassword.resetFailed'))
      }
    } catch {
  ...
  if (done) {
    return (
      <div className="mt-8 flex items-start gap-3 rounded-lg bg-green-50 p-4 text-sm text-green-800">
        ...
        <div>
          <p className="font-medium">{t('resetPassword.passwordUpdated')}</p>
          <p className="mt-1">{t('resetPassword.redirectingToLogin')}</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex min-h-screen">
      <div className="flex w-full flex-col justify-center px-8 lg:w-1/2 lg:px-16">
        <div className="mx-auto w-full max-w-sm">
          ...
          <Suspense fallback={<div className="mt-8 text-sm text-gray-500">加载中...</div>}>
            <ResetPasswordForm />  {/* t is used inside */}
          </Suspense>
      ...
      <div className="hidden w-1/2 bg-gradient-to-br from-blue-600 to-indigo-700 lg:flex lg:flex-col lg:justify-center lg:px-16">
        <div className="mx-auto max-w-md text-white">
          <h2 className="text-3xl font-bold">{t('resetPassword.almostDone')}</h2>
          <p className="mt-4 text-lg text-blue-100">{t('resetPassword.almostDoneDesc')}</p>
        </div>
      </div>
    </div>
  )
