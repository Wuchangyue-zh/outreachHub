...
/**
 * GET/POST /api/cron/advance-sequences
 *
 * #7: 推进 SEQUENCE 类型 Campaign 的后续步骤。
 * Vercel Cron 使用 GET；本地可 POST。
 */
export async function GET(req: NextRequest) {
  return handleAdvanceSequences(req)
}
...
async function handleAdvanceSequences(req: NextRequest) {
  try {
    ...
    if (campaigns.length === 0) {
      return NextResponse.json({ success: true, data: { processed: 0, message: '没有进行中的序列活动' } })
    }
    ...
    for (const campaign of campaigns) {
      ...
      for (const cid of contactIds) {
        ...
        if (count < minCompletedSteps) {
          minCompletedSteps = count
        }
      }
      ...
      const emailJobs = contacts
        .map((contact) => {
          ...
          const subject = applyEmailVariables(currentStep.subject || campaign.subject, vars)
          const html = applyEmailVariables(rawHtml, vars)
          const text = applyEmailVariables(currentStep.content || '', vars)
    ...
    return NextResponse.json({ success: true, data: { processed: campaigns.length, results } })
  } catch (error) {
    console.error('[advance-sequences] Error:', error)
    return NextResponse.json({ success: false, error: '序列推进失败' }, { status: 500 })
  }
}
